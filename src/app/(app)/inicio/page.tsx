...
const INICIO_POR_PAPEL: Record<Exclude<Papel, "enfermeira">, { titulo: string; texto: string; acao?: { rotulo: string; href: string } }> = {
  comercial: {
    titulo: "A fila do dia vai aparecer aqui",
    texto: "Transferências da Isadora por prioridade e prazo, com o botão para assumir, e as tarefas que vencem hoje.",
    acao: { rotulo: "Ver transferências", href: "/transferencias" },
  },
  coordenacao: {
    titulo: "O que pede decisão vai aparecer aqui",
    texto: "Alertas clínicos abertos, transferências de saúde, fichas pendentes, ofertas sem resposta e a síntese da equipe.",
  },
  financeiro: {
  ...
  marketing: {
    ...
    texto: "Leads por origem e por estágio, sempre em agregado, sem dado de família.",
  },
  diretoria: {
    titulo: "As cinco perguntas do dia vão aparecer aqui",
    texto: "Vendas, operação, equipe, financeiro e alertas, cada uma com a comparação ao período anterior.",
...
export default async function PaginaInicio() {
  ...
  const conteudo = INICIO_POR_PAPEL[principal && principal !== "enfermeira" ? principal : "comercial"];
  return (
    <TelaEmConstrucao
      titulo="Início"
