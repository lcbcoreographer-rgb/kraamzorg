...
export function parametro(busca: Record<string, string | string[] | undefined>, chave: string) {
  const valor = busca[chave];
  return Array.isArray(valor) ? valor[0] : valor;
}

/** ?proximo= só quando é caminho interno do app. */
export function proximoDaBusca(busca: Record<string, string | string[] | undefined>) {
