...
import { PAPEIS, ROTULO_PAPEL } from "@/lib/auth/papeis";
import {
...
function ErroGeral({ estado }: { estado: EstadoFormulario }) {
  if (!estado.erro) return null;
  return <FaixaAlerta variante="imediato" titulo={estado.erro} anunciar />;
}
...
export function FormularioEntrar({ proximo }: { proximo?: string }) {
  ...
  return (
    <form action={acao} className="flex flex-col gap-6" noValidate>
      ...
      />
      <Botao type="submit" largaTotal carregando={enviando} rotuloCarregando="Entrando">
        Entrar
      </Botao>
...
export function FormularioDesafio({ proximo }: { proximo?: string }) {
  ...
  return (
    <form action={acao} className="flex flex-col gap-6" noValidate>
      <ErroGeral estado={estado} />
      {proximo ? <input type="hidden" name="proximo" value={proximo} /> : null}
      <CampoCodigo erro={estado.errosCampo?.codigo} ajuda="O código muda a cada 30 segundos." />
      <Botao type="submit" largaTotal carregando={enviando} rotuloCarregando="Conferindo">
        Confirmar e entrar
      </Botao>
    </form>
  );
...
export function FormularioCadastroMfa({ fatorId, proximo }: { fatorId: string; proximo?: string }) {
  const [estado, acao, enviando] = useActionState(confirmarCadastroMfa, inicial);
  return (
    <form action={acao} className="flex flex-col gap-6" noValidate>
      <ErroGeral estado={estado} />
  ...
  return (
    <form action={acao} className="flex flex-col gap-6" noValidate>
      ...
      <CampoTexto
        ...
        required
      />
      <Botao type="submit" largaTotal carregando={enviando} rotuloCarregando="Enviando">
...
export function FormularioDefinirSenha() {
  ...
  return (
    <form action={acao} className="flex flex-col gap-6" noValidate>
      ...
      />
      <Botao type="submit" largaTotal carregando={enviando} rotuloCarregando="Salvando">
        Salvar senha
      </Botao>
    </form>
  );
...
export function FormularioConvite() {
  ...
  return (
    <form action={acao} className="flex flex-col gap-6" noValidate>
      ...
      <EscolhaMultipla
        rotulo="Papéis"
        descricao="Uma pessoa pode ter mais de um papel. Enfermeira, financeiro, coordenação e diretoria entram sempre com o código do aplicativo."
        name="papeis"
        opcoes={PAPEIS.map((papel) => ({ valor: papel, rotulo: ROTULO_PAPEL[papel] }))}
        erro={estado.errosCampo?.papeis}
      />
      <Botao type="submit" largaTotal carregando={enviando} rotuloCarregando="Enviando convite">
        Enviar convite
      </Botao>
    </form>
  );
