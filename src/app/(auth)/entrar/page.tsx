...
import { parametro, proximoDaBusca, type ParametrosBusca } from "../_componentes/parametros";
 ...
 */
export default async function PaginaEntrar({ searchParams }: { searchParams: ParametrosBusca }) {
  ...
  return (
    <>
      <h1 className="font-titulo text-display text-texto font-normal">Entrar</h1>
      {aviso ? <FaixaAlerta variante="info" titulo={aviso} /> : null}

      {forma.tipo === "senha" ? (
        <FormularioEntrar proximo={proximo} />
      ) : (
        <section aria-labelledby="titulo-seletor" className="flex flex-col gap-4">
          <FaixaAlerta variante="info" titulo="Modo demonstração">
            Dados fictícios, só neste computador. Escolha com quem entrar; nada aqui toca o banco
            de verdade.
          </FaixaAlerta>
          <h2 id="titulo-seletor" className="font-titulo text-2 text-texto font-medium">
            Entrar como
          </h2>
            ...
            {forma.opcoes.map((opcao) => (
              <li key={opcao.usuarioId}>
                <form action={entrarPorSeletor}>
                  <input type="hidden" name="usuarioId" value={opcao.usuarioId} />
                  ...
                  >
                    <UserRound aria-hidden="true" className="text-texto-2 size-5 shrink-0" strokeWidth={1.75} />
                    <span className="flex flex-col">
                      <span className="text-corpo text-texto font-semibold">
                        {descreverPapeis(opcao.papeis)}
                      </span>
                      <span className="text-apoio text-texto-2">{opcao.nome}</span>
                    </span>
                  </button>
                </form>
              </li>
