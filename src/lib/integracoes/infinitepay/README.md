...
- `limites.ts`: o limite de parcelas sem juros (PRD 14 v4.2, T-06), lido
  de `pacote_versao.parcelas_max_sem_juros` por quem chama (3 no seed) e
  validado antes de qualquer chamada de rede. Nenhum número no código.
...
- `webhook.ts`: `processarWebhookInfinitePay`, a lógica por trás da rota
  ...
  por `payment_check`, e só `paid: true` confirma (`success: true` diz só
  que a consulta funcionou). O valor pago também vem do `payment_check` e
  precisa cobrir o valor da cobrança. É idempotente: cobrança já paga não
  baixa de novo, e a baixa é condicional no banco.

Configuração só em variável de ambiente: `INFINITEPAY_HANDLE` e
`INFINITEPAY_API_KEY` (opcional, [conferir]) (`.env.example`).
