...
import type { CodigoErroAuth, ProvedorAutenticacao, ResultadoAuth } from "./tipos";
...
function traduzir(erro: ErroGoTrue | null | undefined): CodigoErroAuth {
  ...
  if (erro.status === 429 || erro.code === "over_request_rate_limit") return "limite_tentativas";
  if (erro.code === "weak_password") return "senha_fraca";
  if (erro.code === "invalid_credentials" || erro.status === 400) return "credenciais";
  if (erro.code === "mfa_verification_failed" || erro.code === "mfa_challenge_expired") {
    return "codigo_invalido";
  }
  if (erro.code === "otp_expired" || erro.code === "flow_state_expired") return "link_invalido";
  if (erro.status === undefined || erro.status >= 500) return "indisponivel";
  return "desconhecido";
}

const ok: ResultadoAuth = { ok: true };
const falha = (erro: ErroGoTrue | null | undefined): ResultadoAuth => ({ ok: false, erro: traduzir(erro) });
...
export function criarAutenticacaoSupabase(): ProvedorAutenticacao {
  ...
  return {
    ...
    async iniciarCadastroMfa() {
      ...
      return { fatorId: data.id, qrCodeSvg: data.totp.qr_code, segredo: data.totp.secret };
    },

    async confirmarCadastroMfa(fatorId, codigo) {
      const { error } = await (await cliente()).auth.mfa.challengeAndVerify({
        factorId: fatorId,
        code: codigo,
      });
      return error ? falha(error) : ok;
    ...
    async enviarRecuperacaoSenha(email, urlRetorno) {
      const { error } = await (await cliente()).auth.resetPasswordForEmail(email, {
    ...
    async definirSenha(novaSenha) {
      const { error } = await (await cliente()).auth.updateUser({ password: novaSenha });
      return error ? falha(error) : ok;
    },

    async confirmarLinkEmail(tokenHash, tipo) {
      const { error } = await (await cliente()).auth.verifyOtp({ token_hash: tokenHash, type: tipo });
