...
/** Abertas sem sessão. */
const PUBLICAS = ["/entrar", "/esqueci-senha", "/auth/confirmar", "/design-system"];

/** Exigem sessão, mas não o AAL2 nem papel (o caminho até o MFA). */
const SO_SESSAO = ["/mfa/cadastro", "/mfa/desafio", "/definir-senha", "/sair"];
...
export function proximoSeguro(proximo: string | null | undefined): string | null {
...
export function decidirAcesso(caminho: string, sessao: SessaoBorda | null): DecisaoAcesso {
  const seguir: DecisaoAcesso = { tipo: "seguir" };
  const para = (destino: string): DecisaoAcesso => ({ tipo: "redirecionar", para: destino });

  if (PUBLICAS.some((base) => casa(caminho, base))) {
    // Quem já entrou e abre /entrar vai direto para o início (ou para o MFA).
    if (sessao && caminho === "/entrar" && sessao.ativo && sessao.papeis.length > 0) {
  ...
  if (casa(caminho, "/sair")) return seguir;
  ...
  if (SO_SESSAO.some((base) => casa(caminho, base))) {
    ...
    if (!mfa && casa(caminho, "/mfa")) return para(caminhoInicial(sessao.papeis));
    // Tem MFA cadastrado e abriu o cadastro: vai para o desafio.
    if (mfa === "/mfa/desafio" && casa(caminho, "/mfa/cadastro")) return para(mfa);
    return seguir;
  }
  ...
  const restrita = Object.entries(RESTRITAS).find(([base]) => casa(caminho, base));
  if (restrita) {
  ...
  if (podeAbrir(sessao.papeis, caminho) === false) return para(caminhoInicial(sessao.papeis));
 ...
 * vai para o cadastro.
 */
export function precisaMfa(sessao: SessaoBorda): "/mfa/desafio" | "/mfa/cadastro" | null {
