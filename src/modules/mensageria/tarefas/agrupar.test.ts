...
describe("agruparTarefas", () => {
  it('"hoje" é o dia em Brasília, qualquer que seja o fuso do servidor', () => {
    // 22h em Brasília já é o dia seguinte em UTC.
    const noite = new Date("2026-09-24T22:00:00-03:00");
    const grupos = agruparTarefas(
      [
        tarefa({ id: "ainda-hoje", venceEm: "2026-09-24T23:30:00-03:00" }),
        tarefa({ id: "madrugada", venceEm: "2026-09-25T01:00:00-03:00" }),
      ],
      noite,
    );
    expect(
      grupos.find((g) => g.balde === "vence_hoje")?.tarefas.map((t) => t.id),
    ).toEqual(["ainda-hoje"]);
    expect(
      grupos.find((g) => g.balde === "a_vencer")?.tarefas.map((t) => t.id),
    ).toEqual(["madrugada"]);
  });
  ...
  it("agrupa em vencida, vence_hoje, a_vencer e sem_prazo, só com os baldes usados", () => {
    ...
    expect(
      grupos.find((g) => g.balde === "vencida")?.tarefas.map((t) => t.id),
    ).toEqual(["vencida"]);
  });
    ...
    const grupos = agruparTarefas(
      [
        tarefa({
          id: "normal-cedo",
          prioridade: "normal",
          venceEm: "2026-09-25T09:00:00-03:00",
        }),
        tarefa({
          id: "maxima",
          prioridade: "maxima",
          venceEm: "2026-09-25T18:00:00-03:00",
        }),
        tarefa({
          id: "alta",
          prioridade: "alta",
          venceEm: "2026-09-25T10:00:00-03:00",
        }),
      ],
      AGORA,
    );
    const aVencer = grupos.find((g) => g.balde === "a_vencer");
    expect(aVencer?.tarefas.map((t) => t.id)).toEqual([
      "maxima",
      "alta",
      "normal-cedo",
    ]);
...
describe("textoPrazo", () => {
  ...
  it("minutos, para dentro de uma hora", () => {
    expect(textoPrazo("2026-09-24T15:22:00-03:00", AGORA)).toBe(
      "vence em 22 min",
    );
    expect(textoPrazo("2026-09-24T14:52:00-03:00", AGORA)).toBe(
      "venceu há 8 min",
    );
  });
  ...
  it("data e hora completas, para mais de um dia de distância", () => {
    expect(textoPrazo("2026-09-30T10:00:00-03:00", AGORA)).toMatch(
      /^até 30\/09\/2026/,
    );
  });
});
