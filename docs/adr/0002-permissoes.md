...
| `api.buscar_duplicatas_pipeline()` (0017, P17) | comercial, coordenação, diretoria | pela regra do perfil | não | Todos os pares entre famílias não mescladas, pela regra de `privado.buscar_duplicatas`: `certas`, `provaveis` e `novasGestacoes` (a mais recente em `a`), cada família só com id, nome, bairro, cidade e DPP |
| `api.mesclar_familias(familia_fica_id, familia_perde_id, oportunidade_fica_id)` (0017, P17) | comercial, diretoria (quem altera `familia` e move o P1; a coordenação só lê família) | pela regra do perfil | não (log `mesclagem` só com ids e contagens, evento na ficha) | Move oportunidades, conversas, transferências, tarefas e sessões de venda; com oportunidade aberta nas duas, a outra vai para `perdido` (outro, "mesclada em <id>") por `privado.transicionar`; a perdida no P1 continua na família que sai (índice de oportunidade aberta); herda freio (com a data e o autor do freio original, então quem mesclou não ganha o "Desfazer" nem a justificativa de um freio que não acionou), `historico_sensivel` e não contatar. Recusa família que sai com contrato, acompanhamento ou consulta pré-natal |
| `api.vincular_nova_gestacao(familia_id, familia_anterior_id)` (0017, P17) | comercial, diretoria | pela regra do perfil | não (auditoria do update e evento `nova_gestacao`) | Chama `privado.vincular_nova_gestacao` |
| `api.pode_enviar_mensagem(familia_id, categoria, canal)` (0017, P18) | comercial, enfermeira, financeiro, coordenação, diretoria, só em família que o usuário vê (`privado.tem_acesso_familia`) | pela regra do perfil | não | `{pode, motivo}` de `privado.pode_enviar_mensagem`, sem mudança |
| `api.registrar_envio_tarefa(tarefa_id, texto)` (0017, P18) | quem responde pela tarefa (mesma regra da RLS de `tarefa`) e vê a família | pela regra do perfil | não (auditoria do insert da mensagem, conteúdo oculto) | Reconsulta o freio (canal manual), grava a mensagem humana mascarada na conversa iniciada pela família e conclui a tarefa, numa transação. Recusa do freio: `P0001` com o código |
| `api.pausar_conversa(conversa_id, motivo)` e `api.retomar_pausa_conversa(conversa_id)` (0017, P27) | comercial, coordenação, diretoria | pela regra do perfil | não (log `agente_pausado` e `agente_pausa_retomada`) | Pausa por `agente_pausa_humano_horas` (nunca encurta); retomar limpa só a pausa, nunca o `humano_comercial` |
| `api.resolver_transferencia(handoff_id, desfecho)` (0017, P27) | comercial, coordenação, diretoria | pela regra do perfil | não (log `transferencia_resolvida`) | Desfecho em `handoff.dados.desfecho`; nunca devolve a conversa à Isadora (só `api.retomar_agente`) |
| `api.reenviar_notificacao_handoff(handoff_id)` (0017, P27) | comercial, coordenação, diretoria | pela regra do perfil | não (log `handoff_aviso_reenviado`) | Notificação ao papel do destino (app, whatsapp_interno, email) e `notificacao_ok` nulo |
| `api.base_conhecimento_listar()` e `api.base_conhecimento_salvar(id, tipo, titulo, texto, fonte)` (0017, P27) | comercial, coordenação, diretoria | pela regra do perfil | não (auditoria do gatilho) | Itens de `agente.base_conhecimento`; toda edição volta para rascunho sem aprovação |
| `api.base_conhecimento_aprovar(id)` (0017, P27) | diretoria | sim, sempre | não (auditoria do gatilho) | Grava `aprovado`, `aprovado_por` e `aprovado_em` |
| `api.ultima_ingestao_base()` (0017, P27) | comercial, coordenação, diretoria | pela regra do perfil | não | `{em, ok, itens, erro}` da execução mais recente de `agente.ingestao_execucao`; nulo antes da primeira |
| `api.metricas_agente(desde, ate)` (0017, P27) | comercial, coordenação, diretoria | pela regra do perfil | não | Só agregados do PRD 11.12 |
...
- **`anon`:** nenhuma.
- **`authenticated`:** `privado.tem_papel`, `privado.familias_atribuidas`, `privado.aal2`, `privado.sem_acento` e as trinta e quatro funções de `api` da seção 5 (`api.transicionar`, `api.familias_do_dia`, `api.ficha_assistencial`, `api.dados_contrato`, `api.status_cobranca`, `api.sessao_venda_gravacao`, `api.status_equipe`, `api.marketing_leads_por_origem`, `api.marketing_funil`, `api.lead_origem`, `api.log_auditoria` e, desde o P09, `api.acionar_freio`, `api.desfazer_freio`, `api.justificar_freio` e `api.reverter_freio`; desde o P22, `api.retomar_agente` e `api.eliminar_titular`; desde a 0015, `api.revogar_sessoes`, `api.transicoes_permitidas` e `api.parametros_da_tela`; desde a 0017, `api.buscar_duplicatas_pipeline`, `api.mesclar_familias`, `api.vincular_nova_gestacao`, `api.pode_enviar_mensagem`, `api.registrar_envio_tarefa`, `api.pausar_conversa`, `api.retomar_pausa_conversa`, `api.resolver_transferencia`, `api.reenviar_notificacao_handoff`, `api.base_conhecimento_listar`, `api.base_conhecimento_salvar`, `api.base_conhecimento_aprovar`, `api.ultima_ingestao_base` e `api.metricas_agente`). As funções do freio, da pontuação, da deduplicação, da ocupação e do recálculo em `privado` (0009 a 0011) não entram: o app chega a elas pelos wrappers, e o agente pelo schema `agente` (P21).
...
| Mesclagem de famílias (0017) | Comercial e diretoria (a tela do P17 também deixava a coordenação, que na matriz só lê família); perdida no P1 continua na família que sai; família que sai com contrato, acompanhamento ou consulta pré-natal não é mesclada | Leonardo |
| Base de conhecimento (0017) | Leitura e edição de comercial, coordenação e diretoria; aprovação só da diretoria em AAL2; toda edição volta para rascunho | Leonardo |
