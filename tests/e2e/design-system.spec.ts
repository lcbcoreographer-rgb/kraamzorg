...
test.describe("/design-system", () => {
  test("carrega e mostra o título da direção visual", async ({ page }) => {
    ...
    await expect(page.getByRole("heading", { name: "Caderneta de visita", level: 1 })).toBeVisible();
  });

  test("não tem violação de acessibilidade séria ou crítica (axe)", async ({ page }) => {
    await page.goto("/design-system");
